...
> 📌 Some commands could not be working since CrackMapExec is no longer maintained -> Jump to [netexec](#netexec) for working commands
...
![Dump SAM and Shares](.gitbook/assets/2024-08-16_09-26-19_687.png)
...
![netexec lsassy output](.gitbook/assets/2024-08-16_09-44-46_688.png)
...
Then, open the `\\hydra-dc\hackme` share folder in `THEPUNISHER` VM and check the Responder log for hashes automatically captured.

![Responder capturing hashes](.gitbook/assets/2025-02-15_09-35-18_881.png)

**Automated attack** - if the file share is exposed, use `netexec` [slinky](https://github.com/seriotonctf/cme-nxc-cheat-sheet?tab=readme-ov-file#slinky) built-in module to create the link/shortcut file on the targeted VM (in all shares with write permissions).
...
```bash
# [Commands cut off in the provided notes]
```

[Note: This section is incomplete in the provided notes.]